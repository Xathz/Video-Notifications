...
namespace VideoNotifications {
    ...
    public partial class MainForm {
        ...
        public void AddChannelVideos(string channelID) {
            ...
            IEnumerable<Database.CollectionType.YouTubeVideo> videos = Database.Channels.GetAllVideos(channelID).OrderByDescending(v => v.Posted);
        ...
        private void AddChannelToListView(Database.CollectionType.YouTubeChannel channel) {
            if (!ChannelsImageList.Images.ContainsKey(channel.ChannelID)) {
                Image resizedImage = ImageUtils.ResizeImage(Database.Files.GetThumbnail(channel.ChannelID), 24, 24);
        ...
        private void AddVideoToListView(Database.CollectionType.YouTubeVideo video) {
        ...
        private void UpdateVideoInListView(Database.CollectionType.YouTubeVideo video) {
            ListViewItem item = VideosListView.Items.Find(video.VideoID, false).FirstOrDefault();
            if (item != null) {
                Database.CollectionType.YouTubeVideo updatedVideo = Database.Videos.GetByID(video.VideoID);
        ...
        private void SetStatus(Database.CollectionType.Status status) {
            if (ModifierKeys == Keys.Shift) {
                if (MessageBox.Show($"Really mark all videos as {status.ToString()}?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes) {
                    ...
                    foreach (ListViewItem item in VideosListView.Items) {
                        if (((Database.CollectionType.YouTubeVideo)item.Tag).Status != status) {
                            Database.Videos.SetStatus(item.Name, status);
                            UpdateVideoInListView((Database.CollectionType.YouTubeVideo)item.Tag);
                        }
                    }
                    VideosListView.EndUpdate();
                }
                ...
                if (VideosListView.SelectedItems.Count == 1) {
                    ...
                    UpdateVideoInListView((Database.CollectionType.YouTubeVideo)selectedItem.Tag);
                }
            }
        }
        ...
        private dynamic DisplayStatusColor(Database.CollectionType.Status status) {
            switch (status) {
                case Database.CollectionType.Status.Watched:
                    return Color.LightGreen;
                case Database.CollectionType.Status.Unwatched:
                    return SystemColors.Window;
                case Database.CollectionType.Status.Dismissed:
                    return Color.LemonChiffon;
                case Database.CollectionType.Status.Ignored:
